...
import { MicroEvidenceItem, MasterScorecardReport, FinalVerdict } from "../types";
...
1. SINGLE SOURCE OF TRUTH:
   - The "AI Evaluator Observations" MUST STRICTLY derive 100% from the detected events in the "Grounded Micro-Evidence Timeline".
...
2. ZERO-TOLERANCE PPE SAFETY ENFORCEMENT:
   - If bare skin, lack of 1000V dielectric gloves, or missing eye protection is detected at ANY timestamp during live/de-energization procedures (e.g. 14.2s):
     ...
     * FORBIDDEN HALLUCINATION: You are STRICTLY FORBIDDEN from stating "Zero electrocution hazard: Class 0 1000V gloves detected throughout isolation step" if a glove failure occurred.
     * The corresponding observation MUST state: "CRITICAL PPE VIOLATION: Missing 1000V insulated gloves at [timestamp]. Direct electrocution risk logged (-25 pts penalty)."
...
4. CONCISE CRITICAL FEEDBACK:
   - Limit observations to a maximum of 3-4 concise, highly critical, evidence-grounded bullet points.
...
export function reconcileVisionEvaluation(
  ...
  let ppeScore = 96.5;
  if (gloveViolation) {
    // Zero tolerance: force score down based on penalty
    const penalty = gloveViolation.penalty_points || 25.0;
    ppeScore = Math.max(50.0, Math.min(68.0, 100.0 - penalty - 10.0)); // 65.0%
  }
  ...
  if (gloveViolation) {
    ...
    notes.push(
      `CRITICAL PPE SAFETY VIOLATION: Bare-hand contact flagged at ${timeSec}s (-${gloveViolation.penalty_points || 25} pts). Missing Class 0 1000V dielectric gloves during live terminal handling.`
    );
  } else {
    notes.push(
